// 11_echo_client.cpp : 定义控制台应用程序的入口点。
//
...
private:
	...
	void do_read() {
		async_read(sock_, buffer(read_buffer_),
			MEM_FN2(read_complete, _1, _2), MEM_FN2(on_read, _1, _2)); // 异步读
	}
	void do_write(const std::string & msg) {
		if (!started()) return;                                    // 如果停止发送，返回
		std::copy(msg.begin(), msg.end(), write_buffer_);          // 拷贝带发送消息到发送缓存
		sock_.async_write_some(buffer(write_buffer_, msg.size()),  // 异步写，写完传入读回调
